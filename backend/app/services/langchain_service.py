...
class EnhancedLangChainService:
    ...
    def _initialize_prompts(self):
        ...
        # System prompts for different modes — adapted for SLB-B educational context
        self.system_prompts = {
            ConversationMode.CASUAL: """Anda adalah asisten belajar untuk siswa Sekolah Luar Biasa tipe B (SLB-B / tunarungu) yang ramah dan membantu.
            Berikan penjelasan materi pelajaran yang jelas dan mudah dipahami. Gunakan bahasa sederhana, kalimat pendek, dan tunjukkan kesabaran serta empati.
            WAJIB SELALU jawab dalam bahasa Indonesia yang baik dan benar.""",
            ConversationMode.FORMAL: """Anda adalah asisten belajar profesional untuk siswa SLB-B (Sekolah Luar Biasa tipe B / tunarungu).
            Berikan penjelasan materi pelajaran yang formal, akurat, dan komprehensif sesuai kurikulum.
            Gunakan terminologi yang tepat dan pertahankan nada edukatif sepanjang interaksi.
            WAJIB SELALU jawab dalam bahasa Indonesia yang baik dan benar.""",
            ConversationMode.TECHNICAL: """Anda adalah asisten ahli materi pelajaran untuk siswa SLB-B (Sekolah Luar Biasa tipe B / tunarungu).
            Berikan penjelasan teknis yang detail sesuai tingkat jenjang (SDLB/SMPLB/SMALB), prosedur, dan konsep kurikulum yang spesifik.
            Sertakan contoh konkret dan langkah-langkah yang relevan jika diperlukan.
            WAJIB SELALU jawab dalam bahasa Indonesia yang baik dan benar.""",
            ConversationMode.EDUCATIONAL: """Anda adalah asisten pendidikan yang membantu siswa SLB-B (Sekolah Luar Biasa tipe B / tunarungu) memahami materi pelajaran.
            Jelaskan konsep dengan jelas, berikan konteks yang sesuai usia dan jenjang, serta tawarkan panduan langkah demi langkah.
            Sertakan contoh dan analogi sederhana untuk membantu siswa memahami materi yang kompleks.
            WAJIB SELALU jawab dalam bahasa Indonesia yang baik dan benar.""",
        }
    ...
    async def _get_institution_context(
        ...
        """Get subject context for typo correction based on subject slug (SLB-B)"""
        return await self._get_subject_context(institution_slug)
    ...
    async def _get_subject_context(
        ...
        try:
            if not subject_slug:
                return "mata pelajaran SLB-B", "SLB-B school subject"
            ...
            async for db in get_db_session():
                ...
                if subject:
                    # Build context from subject data (jenjang + mata_pelajaran)
                    jenjang_label = subject.jenjang or "SLB-B"
                    mp_label = subject.mata_pelajaran or subject.name
                    subject_context = f"pelajaran {mp_label} tingkat {jenjang_label}"
                    if subject.description:
                        subject_context += f" ({subject.description})"

                    subject_context_en = f"{mp_label} subject at {jenjang_label} level"
                    if subject.description:
                        subject_context_en += f" ({subject.description})"
