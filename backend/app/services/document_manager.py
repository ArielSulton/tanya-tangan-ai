...
class DocumentManager:
    ...
    async def add_document(
        ...
        # subject_* params take precedence; institution_* are backwards-compat aliases
        effective_id = subject_id or institution_id
        effective_slug = subject_slug or institution_slug
        ...
        try:
            ...
            # Add subject metadata for tracking
            if effective_id:
                metadata["subject_id"] = effective_id
            if effective_slug:
                metadata["subject_slug"] = effective_slug

            # Determine namespace based on subject slug
            namespace = None
            if effective_slug:
                namespace = f"subject_{effective_slug}"
                logger.info(
            ...
            for key, value in metadata.items():
                if value is not None:
                    ...
                    if key == "author":
                        clean_metadata[key] = effective_slug or "System"
                    elif key == "description":
                        clean_metadata[key] = (
                            f"Document uploaded for {effective_slug or 'general'} subject"
                        )
                    elif key == "title":
        ...
        try:
            ...
            if not self.pinecone_service:
                ...
                fallback_answer = await self._search_local_documents(
                    ...
                    session_id,
                    language,
                    institution_id=effective_id,
                    institution_slug=effective_slug,
                )
                return fallback_answer

            # Log subject-specific search
            if effective_slug:
                logger.info(
                    f"🏫 [DocumentManager] Searching in subject-specific namespace for: {effective_slug}"
                )
            else:
