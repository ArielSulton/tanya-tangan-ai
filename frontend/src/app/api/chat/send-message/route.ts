...
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      ...
      serviceMode,
      inputMethod = 'text',
      // subject_* are the new params; institution_* kept as backwards-compat aliases
      subject_id,
      subject_slug,
      institution_id,
      institution_slug,
    ...
    // Prefer subject_* params; fall back to institution_* for old clients
    const effectiveSubjectId = subject_id ?? institution_id
    const effectiveSubjectSlug = subject_slug ?? institution_slug
    ...
    console.log('🔍 [SendMessage] Received request data:', {
      ...
      subject_slug: effectiveSubjectSlug,
    })
      ...
      if (existingConversation.length === 0) {
        ...
        const newConversation = await db
          ...
          .values({
            ...
            institutionId: effectiveSubjectId ?? null, // subject/institution ID from request
          })
          .returning()
      ...
      try {
        ...
        const qaLogResponse = await fetch(backendQaLogUrl, {
          ...
          body: JSON.stringify({
            ...
            institution_id: effectiveSubjectId ?? 1,
